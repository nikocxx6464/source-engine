...
void CHLSL_Solver_Divide::OnWriteFXC( bool bIsPixelShader, WriteContext_FXC &context )
{
...
void CHLSL_Solver_Divide::Render( Preview2DContext &c )
{
	int type0 = GetSourceVar(0)->GetType();
	int type1 = GetSourceVar(1)->GetType();

	SetUVParamBySourceVar( NPSOP_CALC_DIVIDE, 0, 0 );
	SetUVParamBySourceVar( NPSOP_CALC_DIVIDE, 1, 1 );

	IMaterialVar *var_2 = pEditorRoot->GetUVTargetParam( NPSOP_CALC_DIVIDE, 2 );
	IMaterialVar *var_3 = pEditorRoot->GetUVTargetParam( NPSOP_CALC_DIVIDE, 3 );
	...
	CNodeView::RenderSingleSolver( c, pEditorRoot->GetOperatorMaterial( NPSOP_CALC_DIVIDE ) );
