...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeapon357::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
...
//-----------------------------------------------------------------------------
float CWeapon357::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeapon357::GetMaxRestTime()
{
	if (hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE)
	{
		return 1.5f;
...
// Output :
//-----------------------------------------------------------------------------
bool CWeapon357::StartReload(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeapon357::FinishReload(void)
{
...
void CWeapon357::PrimaryAttack(void)
{
	...
	if (!sde_simple_rifle_bolt.GetInt() && !pHL2Player->Get_R357_Chamber())
	{
		Pump();
	}
	...
	pPlayer->FireBullets( 1, vecSrc, vecAiming, vec3_origin, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
		...
		if (!sde_simple_rifle_bolt.GetInt())
			pHL2Player->R357_Round_Unchamber(); //even if 0 ammo remains, after shot there is no chambered round
